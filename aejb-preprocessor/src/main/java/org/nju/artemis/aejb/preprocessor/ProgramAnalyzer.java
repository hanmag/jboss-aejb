		    ...
		    if ((cn.access & ACC_INTERFACE) == 0) {
		    	...
		    	for (MethodNode mn : (List<MethodNode>) cn.methods) {		    		
		    		...
		    		if (isTransaction(mn,"Ljavax/aejb/Transaction;")) {
		    			...
		    			mn.tryCatchBlocks.add(new TryCatchBlockNode(lb0, lb1, lb2, "javax/naming/NamingException"));
		    			...
		    			begin.add(lb0);		    			
		    			begin.add(new TypeInsnNode(NEW, "javax/naming/InitialContext"));
		    			begin.add(new InsnNode(DUP));
		    			begin.add(new MethodInsnNode(INVOKESPECIAL, "javax/naming/InitialContext", "<init>", "()V"));
		    			begin.add(new VarInsnNode(ASTORE, localnum+1));
		    			
		    			begin.add(lb4);		    			
		    			begin.add(new VarInsnNode(ALOAD, localnum+1));
		    			begin.add(new LdcInsnNode("java:global/aejb/transactionmanager/"+mn.name));
		    			begin.add(new MethodInsnNode(INVOKEINTERFACE, "javax/naming/Context", "lookup", "(Ljava/lang/String;)Ljava/lang/Object;"));
		    			begin.add(new TypeInsnNode(CHECKCAST, "javax/aejb/TransactionTrigger"));
		    			begin.add(new VarInsnNode(ASTORE, localnum));
		    			begin.add(lb1);		    			
		    			begin.add(new JumpInsnNode(GOTO, lb5));
		    			begin.add(lb2);		    			
		    			begin.add(new FrameNode(Opcodes.F_FULL, 2, new Object[] {mn.name, "javax/aejb/TransactionTrigger"}, 1, new Object[] {"javax/naming/NamingException"}));
		    			begin.add(new VarInsnNode(ASTORE, localnum+1));
		    			
		    			begin.add(lb6);		    			
		    			begin.add(new VarInsnNode(ALOAD, localnum+1));
		    			begin.add(new MethodInsnNode(INVOKEVIRTUAL, "javax/naming/NamingException", "printStackTrace", "()V"));
		    			begin.add(lb5);
		    			...
		    			while (i.hasNext()) {
		    			      ...
		    			      if(i1 instanceof MethodInsnNode){
		    			    	 ...
		    			    	 // System.out.println(((MethodInsnNode) i1).owner+","+((MethodInsnNode)i1).name+","+((MethodInsnNode) i1).desc);
		    			    	  if(ejbs.contains(owner)){
		    			    		  String e="Ejb."+owner+"."+((MethodInsnNode)i1).name;
		    			    		  InsnList il = new InsnList();
		    				            il.add(new TypeInsnNode(NEW, "java/lang/StringBuilder"));
		    				            il.add(new InsnNode(DUP));
		    				            il.add(new LdcInsnNode(mn.name+"/"));
		    				            il.add(new MethodInsnNode(INVOKESPECIAL, "java/lang/StringBuilder", "<init>", "(Ljava/lang/String;)V"));
		    				            il.add(new VarInsnNode(ALOAD,0));
		    				            il.add(new MethodInsnNode(INVOKEVIRTUAL, "java/lang/Object", "toString", "()Ljava/lang/String;"));
		    				            il.add(new MethodInsnNode(INVOKEVIRTUAL, "java/lang/StringBuilder", "append", "(Ljava/lang/String;)Ljava/lang/StringBuilder;"));
		    				            il.add(new MethodInsnNode(INVOKEVIRTUAL, "java/lang/StringBuilder", "toString", "()Ljava/lang/String;"));
		    				            insns.insert(i1.getPrevious(),il);		    				            
		    				            //System.out.println(((MethodInsnNode) i1).desc+"111"+((MethodInsnNode) i1).desc.replace("(", "(Ljava/lang/String;"));
			    			    		...
			    			    		trig.add(new MethodInsnNode(INVOKEVIRTUAL,  "javax/aejb/TransactionTrigger", "trigger", "(Ljava/lang/String;Ljava/lang/String;)V"));
		    			    		    ...
		    			    		    insns.insert(i1.getPrevious(),new MethodInsnNode(INVOKEVIRTUAL, ((MethodInsnNode) i1).owner, ((MethodInsnNode) i1).name, ((MethodInsnNode) i1).desc.replace("(", "(Ljava/lang/String;")));
		    			    		    insns.remove(i1);		    			    		    
		    			    	  }
		    			      ...
		    			      else
		    			      {
		    			    	  if(jumpinf.containsKey(i1)){
		    			    		  ...
		    			    		  if(i1.getOpcode()==RETURN){
		    			    			  InsnList trig = new InsnList();
		    				    		  trig.add(new VarInsnNode(ALOAD,localnum));
		    				    		  trig.add(new VarInsnNode(ALOAD,0));
		    				    		  trig.add(new MethodInsnNode(INVOKEVIRTUAL, "java/lang/Object", "toString", "()Ljava/lang/String;"));			    		  
		    				    		  trig.add(new MethodInsnNode(INVOKEVIRTUAL, "javax/aejb/TransactionTrigger", "trigger", "(Ljava/lang/String;)V"));
		 ...
		 for(int i=0;i<stateMachine.getStatesCount();i++){
			 ...
			 for(j=0;j<event.size();j++){
				 if(sn==event.get(j).getHead()){
					 nexts=nexts+event.get(j).getEvent()+"-"+state.indexOf(event.get(j).getTail())+",";					 
				 }
			 }
			 if(nexts.length()>1){
...
//�жϸ÷����Ƿ�����Ҫ�����ķ���
	 ...
	 //��ȡ�����ֽ����control flow 
	 public void ExtractControlFlow(String owner,final MethodNode mn) throws AnalyzerException {
		 ...
		 if(src<stateMachine.getEnd())
		 {
			 ...
			 else
			 {
				 ...
				 if(an instanceof MethodInsnNode){
					...
					/* if(!future.contains(owner)){
						 future.add(owner);
					 }	*/				 
		    		 String e="Ejb."+owner+"."+((MethodInsnNode)an).name;
			    	  if(ejbs.contains(owner)){
			    			  ...
			    			  else
			    			  {
			    				  ...
			    				  if(controlflow.getFlow(src).isWhile)
			    				  {
			    					  for(int k=0;k<controlflow.getDstSize(src);k++){	
		    			    			  ...
		    			    			  if(src<dst){		    			    				  
		    			    				  ...
		    			    				  stateMachine.addEvent(new Event(src,dst, "while("+src+")F"));
		    			    				  jumpinf.put(insns.get(dst), "while("+src+")F");
		    			    				  recognize_state(dst+1,dst,insns);
		    			    			  }
		    			    			  else
		    			    			  {
		    			    				  stateMachine.addState(dst);		    			    			
		    			    				  stateMachine.addEvent(new Event(src,dst, "while("+src+")T"));
		    			    				  jumpinf.put(insns.get(dst), "while("+src+")T");
		    			    				  recognize_state(dst+1,dst,insns);
		    			    			  }
		    			    		  }		    			    
			    				  }
			    				  else
			    				  {
			    					  for(int k=0;k<controlflow.getDstSize(src);k++){	
	    			    				  dst=(Integer)controlflow.getDst(src).get(k);
	    			    				  stateMachine.addState(dst);
	    			    				  stateMachine.addEvent(new Event(src,dst, "if("+src+")"+k));
	    			    				  jumpinf.put(insns.get(dst), "if("+src+")"+k);
	    			    				  recognize_state(dst+1,dst,insns);
	    			    				  }
			    					  }
			    				  }
	 ...
	 public void ExtractMetaData()
	 {
		 int states_count = stateMachine.getStatesCount();
			// ���е�state��ÿ��list����Ǹ�״̬�µ�Tranquillity port
			...
			// ���ȴ�ǰ��������¼��б?Ϊÿһ��״̬���past��˼·�ǣ�����s0-a1-s1��һ�¼���s1��past=a0+s0��past
				...
				for (int i = e.size() - 1; i >= 0; i--) {
					...
					if (tail!= stateMachine.getEnd())
						for (int j = 0; j < future[tailindex].size(); j++) {
							if (!future[headindex].contains(future[tailindex].get(j))) {
	...
	public static void main(String args[])
	{
		try{
			...
			FileInputStream in=new FileInputStream("e:\\Ttest.class");
			showClassSource("e:\\Ttest.class") ;
			...
			//��÷����ļ����ļ���������ɵ��ļ������E:\analize_result��
			//String infilename=args[1].toString();
			String infilename="e:\\Ttest.class";
